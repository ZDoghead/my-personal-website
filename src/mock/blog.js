...
Mock.mock(/^\/api\/blog(\?.+)?$/, 'get', function (options) {
  ...
  const data = { 
    "total|100-600": 786, //总数
    [`rows|${res.limit}`]: [ // 当前页列表数据
      {
        id: '@guid',
        title: "@ctitle",
        ...
        "scanNumber|0-3000": 10,  // 浏览总数
        "commentNumber|0-300": 30,  // 评论总数
...
Mock.mock(/^\/api\/blog\/[^/]+$/, 'get', {
  ...
  data: {
    ...
    createDate: "@date('T')",
    "thumb|1": ["@image('250x200', '#ffcc33', '#FFF', 'png', 'Doghead')", ''],
    toc: [
  ...
  fetch(<span class="hljs-string">"http://crossdomain.com/api/user"</span>, {
    ...
    <span class="hljs-attr">body</span>: <span class="hljs-built_in">JSON</span>.stringify({ <span class="hljs-attr">name</span>: <span class="hljs-string">"茂小州"</span>, <span class="hljs-attr">age</span>: <span class="hljs-number">18</span> }) <span class="hljs-comment">// 设置请求体</span>
  });</code></pre>
  ...
  {"name": "茂小州", "age": 18 }</code></pre><ol start="4">
